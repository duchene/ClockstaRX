...
setMethod("print",
	...
	if(any(grepl("pca", names(x)))){
		...
		for(i in 1:length(x$pca.clock.space$pPCs)){
			...
			cat(paste0("PC", i, " with ", length(which(x$pca.clock.space$pIL[,i] < 0.01)), " significant branches.\n"))
		}
		...
		for(i in 1:length(x$weighted.pca.clock.space$pPCs)){
                        ...
                        cat(paste0("PC", i, " with ", length(which(x$weighted.pca.clock.space$pIL[,i] < 0.01)), " significant branches.\n"))
                }
